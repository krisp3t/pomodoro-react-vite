...
export default function Session(props) {
  ...
  let statusDisplay;
  if (sessionMode.status === SESSION_MODES.initial.status) { statusDisplay = null; } else if (sessionMode.status === SESSION_MODES.breaking.status) { statusDisplay = sessionMode.length; } else { statusDisplay = sessionMode.status; }
    ...
    switch (action.type) {
      ...
      case 'COMPLETED':
        ...
        if (state.status === SESSION_MODES.working.status) {
          ...
          if (action.settingsCtx.isNotifications) { displayNotification(SESSION_MODES.breaking.status); }
          if (action.numBreaks % 3 === 0) {
            return {
              ...SESSION_MODES.longBreak,
              ...start,
              sessionLength: action.settingsCtx.longBreakDuration,
            };
          }
          return {
            ...SESSION_MODES.shortBreak,
            ...start,
            sessionLength: action.settingsCtx.shortBreakDuration,
          };
        }
        setTimeout(() => props.addItem({ ...state, end: Date.now() }), 0);
        if (action.settingsCtx.isNotifications) { displayNotification(SESSION_MODES.working.status); }
        return { ...SESSION_MODES.working, ...start, sessionLength: action.settingsCtx.pomodoroDuration };
    ...
    if (SESSION_MODES.working.status === sessionMode.status) {
      id = setInterval(() => {
        ...
        if (passedTime >= settingsCtx.pomodoroDuration) {
          numBreaks.current++;
          dispatchMode({ type: 'COMPLETED', numBreaks: numBreaks.current, settingsCtx });
        } else {
      ...
      id = setInterval(() => {
        ...
        if (passedTime >= breakTime) {
          dispatchMode({ type: 'COMPLETED', settingsCtx });
        } else {
